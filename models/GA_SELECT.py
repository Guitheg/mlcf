...
class GA_SELECT(SuperModule):
    def __init__(
        ...
        **kwargs
        ...
        self.loss_function = function  # setable after init to customize

        self.ga_model = None
        print("population_size", self.population_size)

    def base_loss_function(X):
        global glb_raw_data
        if X.sum() == 0.0:
            loss = 1000
            return loss

        selected_data = (glb_raw_data.copy()).loc[:, (X != 0.0)]
        print(selected_data)
        loss = 0
    ...
    def create_ga_model(self):
        ...
        if self.loss_function is None:
            loss_f = self.base_loss_function
        else:
            loss_f = self.loss_function

        print("self.ndim ",self.ndim)
        print("self.ndim type ", type(self.ndim))
        ga_model = ga(
            function=loss_f, variable_type='bool',
            dimension=self.ndim,
    ...
    def create_ts_data(self, input_width, data=None):
        if data is None:
            data = self.data
