...
Example Usage:
```python
# If we don't want to use the Data Interval Module.
# We can simple use a WTSeries with our data.
...
class WTSeries(WindowIterator):
    ...
    This class inherit of WindowIterator which allow us to iterate over a time series dataframe with a sliding window.
    The WTSeries contains {data} which is the Windowed Time Series. It is a multi-index pandas.DataFrame with a WindowIndex and a TimeIndex.
    
    $$A* = sum_{i} a_i$$
    
    WTSeries build the Windowed Time Series. So the iteration is performed on the WindowIndex of the dataframe.

    Attributes:
        data (pandas.DataFrame): A multi-index windowed pandas.DataFrame with a WindowIndex and a TimeIndex.
    ...
    def __init__(self, data: pd.DataFrame):
        """
        The constructor suppose you already built the windowed time series.
        ...
        Raises:
            IncompatibleDataException: If the both WindowIndex and TimeIndex are not the index of the dataframe.
        """
    ...
    def create_wtseries(
        ...
        Args:
            ...
            selected_columns (Optional[List[str]], optional): The list of name of features we want to keep in the WTSeries.
                If None is given, then every feature will be kept. Defaults to None. Defaults to None.

            window_filter (Optional[WindowFilter], optional): An object which inherit of WindowFilter.
            This allows to filter windows according to some condition. For example, filter the window in order to
            uniformise the histogram of a given column. If None, any window filtering is performed. Defaults to None.

            std_by_feature (Optional[Dict[str, StandardisationFct]], optional): A dict which give us the information about what standardisation apply to each
                columns. 
                ...
                If None, any standardisation is apply. Defaults to None.. Defaults to None.

        Raises:
            DataEmptyException: Raise this exception if the time series dataframe is null or if the window width is to large.
    ...
    def merge(self, wtseries: WTSeries) -> WTSeries:
        """
        Merge a wtseries to the current WTSeries. It will add all the window of the given wtseries to the current WTSeries.

        Args:
            wtseries (WTSeries): The WTSeries we want to merge on the current WTSeries.

        Returns:
            WTSeries: Return the result of the merge operation between the current and the given wtseries.
        """
    ...
    def write(self, dirpath: Path, filename: str, group_file_key: str = None) -> Path:
        ...
        Args:
            ...
            group_file_key (str, optional): If we want to save the current data in a namespace of the file. Defaults to None.
    ...
    def read(filepath: Path, group_file_key: str = None) -> WTSeries:
        ...
        Args:
            ...
            group_file_key (str, optional): The namespace which contains the wanted data. Defaults to None.
